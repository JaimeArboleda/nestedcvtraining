...
from .pipes_and_transformers import MidasPipeline, MidasEnsembleClassifiersWithPipeline
from imblearn.pipeline import Pipeline
from sklearn.calibration import CalibratedClassifierCV
from copy import deepcopy
from .metrics import evaluate_metrics, average_metrics
from .reporting import write_train_report, ordered_keys_classes, prop_minority_class, MetadataFit, averaged_metadata_list
import numpy as np
from collections import Counter

"""TODO
- Cambiar funciones de gráficas para que reciban listados y devuelvan única imagen.
- Usar el k_max_undersampling en los parámetros de search space.
- Usar la opción de build_model = False.
- Hacer opción de no calibrado
- En el reporting, añadir la fila de medias de las métricas
...
def train_model_without_undersampling(model, X, y, exists_resampler):
    ...
    if exists_resampler: # In this case, model is a pipeline with a resampler.
        metadata = fold_model.get_metadata_fit()
    else:
        metadata = MetadataFit(len(y), prop_minority_class(Counter(y)))
    return fold_model, metadata


def train_ensemble_model_with_undersampling(model, X, y, exists_resampler):
    ...
    # K is the imbalanced ratio round to int (with a minimum of 2)
    imbalance_ratio = counter_classes[majority_class_key] / counter_classes[minority_class_key]
    ...
    for _, index in kf.split(majority_class_idxs):
        ...
        if exists_resampler:  # In this case, model is a pipeline with a resampler.
            list_metadata.append(fold_model.get_metadata_fit())
        else:
            list_metadata.append(MetadataFit(len(y_train_f), prop_minority_class(Counter(y_train_f))))
...
def ensemble_binary_model_with_resampling(X, y, pipeline_post_process, model, loss_metric,
                                          peeking_metrics, k_inner_fold, skip_inner_folds,
                                          undersampling_majority_class):

    complete_steps = pipeline_post_process.steps + [('model', model)]
    complete_pipeline = MidasPipeline(complete_steps)
    fold_models = []
    fold_metrics = []
    list_metadata = []
    comments = {}
    ...
    for k, (train_index, test_index) in enumerate(inner_cv.split(X, y)):
        if k not in skip_inner_folds:
            ...
            calibrated_model = CalibratedClassifierCV(fold_base_model, method='isotonic', cv='prefit')
            calibrated_model.fit(X_test, y_test)
            fold_models.append(calibrated_model)
            y_proba = calibrated_model.predict_proba(X_test)[:, 1]
            fold_metrics.append(evaluate_metrics(y_test, y_proba, loss_metric, peeking_metrics))
    ...
    comments['average size of training set before resampling'] = metadata.get_num_init_samples_bf()
    comments['average prop of minority class before resampling'] = metadata.get_prop_minority_class_bf()
    comments['average size of training set after resampling'] = metadata.get_num_init_samples_af()
    comments['average prop of minority class after resampling'] = metadata.get_prop_minority_class_af()
    return complete_model, averaged_metrics, comments


def ensemble_binary_model_without_resampling(X, y, pipeline_post_process, model, loss_metric,
                                             peeking_metrics, k_inner_fold, skip_inner_folds,
                                             undersampling_majority_class):
    ...
    for k, (train_index, test_index) in enumerate(inner_cv.split(X_t, y_t)):
        if k not in skip_inner_folds:
            X_train, y_train, X_test, y_test = X_t[train_index], y_t[train_index], X_t[test_index], y_t[test_index]
            if undersampling_majority_class:
                fold_base_model, fold_metadata = train_ensemble_model_with_undersampling(model, X_train, y_train, False)
            else:
                fold_base_model, fold_metadata = train_model_without_undersampling(model, X_train, y_train, False)
            list_metadata.append(fold_metadata)
            calibrated_fold_model = CalibratedClassifierCV(fold_base_model, method='isotonic', cv='prefit')
    ...
    averaged_metrics = average_metrics(fold_metrics)
    metadata = averaged_metadata_list(list_metadata)
    complete_model = MidasEnsembleClassifiersWithPipeline(pipeline_post_process, fold_models)
    comments['number of folds'] = len(fold_models)
...
def train_inner_calibrated_binary_model(X, y, X_hold_out=[], y_hold_out=[],
                                        k_inner_fold=2, skip_inner_folds=[], report_doc=None,
                                        n_initial_points=5, n_calls=10,
                                        dict_model_params={}, loss_metric='histogram_width',
                                        peeking_metrics=[], verbose=False, skopt_func=gp_minimize):
    ...
    for key in dict_model_params.keys():
        ...
        exists_resampler = any([hasattr(step_post_process[1], 'fit_resample')
                               for step_post_process in pipeline_post_process.steps])

        @use_named_args(search_space)
        def func_to_minimize(**params):
            copy_params = params.copy()
            undersampling_majority_class = copy_params.pop('undersampling_majority_class', False)
            ...
            if exists_resampler:
                ensemble_model, metrics, comments = ensemble_binary_model_with_resampling(X, y,
                ...
                ensemble_model, metrics, comments = ensemble_binary_model_without_resampling(X, y,
                                                                                             pipeline_post_process,
                                                                                             model,
                                                                                             loss_metric,
                                                                                             peeking_metrics,
            ...
            if len(y_hold_out) > 0:
                ...
                list_holdout_metrics.append(evaluate_metrics(y_hold_out, y_hold_out_proba, loss_metric,
                                                             peeking_metrics))
    ...
    if len(y_hold_out) > 0:
        write_train_report(report_doc, list_params, list_metrics,
                           list_holdout_metrics, peeking_metrics,
                           list_comments)
