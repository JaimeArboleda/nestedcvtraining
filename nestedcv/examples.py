...
if __name__ == "__main__":
    dataset = pd.read_csv('D:/Python/nestedcvtraining/nestedcv/dataset_completo.csv', **{'sep': ";", 'index_col': False, 'decimal': ','})
    ...
    best_model, document = find_best_binary_model(X=X, y=y, model_search_spaces=dict_models,
                                                   verbose=True, k_inner_fold=5, k_outer_fold=5,
                                                   skip_inner_folds=[0, 2], skip_outer_folds=[0, 2, 4],
                                                   n_initial_points=10, n_calls=10, loss_metric='average_precision',
                                                   peeking_metrics=['roc_auc', 'neg_log_loss', 'average_precision', 'neg_brier_score'])
    document.save('report_013v2.docx')
